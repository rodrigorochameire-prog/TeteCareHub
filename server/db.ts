...
export async function listBookings(filters?: { petId?: number; tutorId?: number; status?: string }) {
  ...
  if (filters?.petId) {
    query = query.where(eq(bookings.pet_id, filters.petId)) as any;
  }
  if (filters?.tutorId) {
    query = query.where(eq(bookings.tutor_id, filters.tutorId)) as any;
  }
  if (filters?.status) {
...
export async function getAllCalendarEvents(startDate: Date, endDate: Date) {
  ...
  const bookingsData = await db
    ...
    .from(bookings)
    .leftJoin(pets, eq(bookings.pet_id, pets.id))
    .where(
      and(
