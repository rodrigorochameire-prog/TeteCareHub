...
var HttpError, BadRequestError, UnauthorizedError, ForbiddenError, NotFoundError;
var init_errors = __esm({
  "shared/_core/errors.ts"() {
    ...
    BadRequestError = (msg) => new HttpError(400, msg);
    UnauthorizedError = (msg) => new HttpError(401, msg);
    ForbiddenError = (msg) => new HttpError(403, msg);
    NotFoundError = (msg) => new HttpError(404, msg);
  }
});
...
import { integer, pgTable, text, timestamp, varchar, boolean, jsonb, serial, bigint, uuid } from "drizzle-orm/pg-core";
...
var init_schema = __esm({
  "drizzle/schema.ts"() {
    ...
    pets = pgTable("pets", {
      ...
      food_amount: integer("food_amount"),
    ...
    daycareCredits = pgTable("daycare_credits", {
      id: serial("id").primaryKey(),
      pet_id: integer("pet_id").notNull(),
      package_days: integer("package_days").notNull(),
      // number of days purchased
      package_price: integer("package_price").notNull(),
      // price in cents
    ...
    daycareUsage = pgTable("daycare_usage", {
      id: serial("id").primaryKey(),
      pet_id: integer("pet_id").notNull(),
    ...
    petVaccinations = pgTable("pet_vaccinations", {
      ...
      dose_number: integer("dose_number").default(1),
      // which dose (1, 2, 3...)
    ...
    medicationLibrary = pgTable("medication_library", {
      id: serial("id").primaryKey(),
      name: varchar("name", { length: 200 }).notNull(),
    ...
    preventiveLibrary = pgTable("preventive_library", {
      id: serial("id").primaryKey(),
      name: varchar("name", { length: 200 }).notNull(),
      type: varchar("type", { length: 100 }).notNull(),
    ...
    petMedications = pgTable("pet_medications", {
      ...
      // e.g., "10%", "5mg"
      progression_interval: integer("progression_interval"),
      // adjust every X doses
      target_dosage: varchar("target_dosage", { length: 200 }),
      // target dosage to reach
      current_dose_count: integer("current_dose_count").default(0).notNull(),
      // track dose number for progression
      administration_times: text("administration_times"),
      // JSON array of times in HH:MM format
      is_active: boolean("is_active").default(true).notNull(),
      notes: text("notes"),
      created_at: timestamp("created_at", { withTimezone: true }).defaultNow().notNull()
    });
    dailyLogs = pgTable("daily_logs", {
      id: serial("id").primaryKey(),
      pet_id: integer("pet_id").notNull(),
      ...
      food_consumed: varchar("food_consumed", { length: 50 }),
      // "all", "half", "little", "none"
      feeding_time: varchar("feeding_time", { length: 10 }),
      // HH:MM format
      feeding_amount: varchar("feeding_amount", { length: 100 }),
      // e.g., "200g", "1 xícara"
    ...
    calendarEvents = pgTable("calendar_events", {
      ...
      custom_event_type_id: integer("custom_event_type_id"),
      // Reference to eventTypes table for custom types
      custom_color: varchar("custom_color", { length: 20 }),
      ...
      check_in_date: timestamp("check_in_date", { withTimezone: true }),
      // Start date of stay
      check_out_date: timestamp("check_out_date", { withTimezone: true }),
      // End date of stay
      daily_count: integer("daily_count"),
      // Number of days (calculated)
      // Attached media and logs
      photo_urls: jsonb("photo_urls").$type(),
      // Array of photo URLs
      photo_keys: jsonb("photo_keys").$type(),
      // Array of S3 keys
      log_ids: jsonb("log_ids").$type(),
      ...
      linked_resource_id: integer("linked_resource_id"),
      // ID of the linked resource
      auto_created: boolean("auto_created").default(false).notNull(),
    ...
    photoReactions = pgTable("photo_reactions", {
      id: serial("id").primaryKey(),
      photoId: integer("photoId").notNull(),
      userId: integer("userId").notNull(),
    ...
    subscriptionPlans = pgTable("subscriptionPlans", {
      id: serial("id").primaryKey(),
      name: varchar("name", { length: 100 }).notNull(),
      description: text("description"),
      ...
      validityDays: integer("validityDays").notNull(),
    ...
    fleaTreatments = pgTable("fleaTreatments", {
      id: serial("id").primaryKey(),
    ...
    dewormingTreatments = pgTable("dewormingTreatments", {
      id: serial("id").primaryKey(),
      petId: integer("petId").notNull(),
      productName: varchar("productName", { length: 200 }).notNull(),
      applicationDate: timestamp("applicationDate").notNull(),
    ...
    behaviorRecords = pgTable("behaviorRecords", {
      id: serial("id").primaryKey(),
      petId: integer("petId").notNull(),
    ...
    trainingProgress = pgTable("trainingProgress", {
      id: serial("id").primaryKey(),
      petId: integer("petId").notNull(),
    ...
    bookings = pgTable("bookings", {
      ...
      serviceType: varchar("serviceType", { length: 100 }).notNull().default("creche"),
    ...
    adminInvites = pgTable("admin_invites", {
      id: serial("id").primaryKey(),
    ...
    whatsappConfig = pgTable("whatsappConfig", {
      id: serial("id").primaryKey(),
      apiKey: text("apiKey"),
    ...
    whatsappTemplates = pgTable("whatsappTemplates", {
      id: serial("id").primaryKey(),
    ...
    whatsappConversations = pgTable("whatsappConversations", {
      id: serial("id").primaryKey(),
      userId: integer("userId").notNull(),
    ...
    whatsappMessages = pgTable("whatsappMessages", {
    ...
    whatsappGroups = pgTable("whatsappGroups", {
      id: serial("id").primaryKey(),
      petId: integer("petId").notNull(),
    ...
    whatsappGroupMembers = pgTable("whatsappGroupMembers", {
      id: serial("id").primaryKey(),
    ...
    whatsappAutomations = pgTable("whatsappAutomations", {
      id: serial("id").primaryKey(),
      name: varchar("name", { length: 200 }).notNull(),
    ...
    bookingRequests = pgTable("booking_requests", {
      id: serial("id").primaryKey(),
      petId: integer("petId").notNull(),
      tutorId: varchar("tutorId", { length: 64 }).notNull(),
      // openId from users
      requestedDates: jsonb("requestedDates").$type().notNull(),
    ...
    customPricingPlans = pgTable("custom_pricing_plans", {
      id: serial("id").primaryKey(),
      tutorId: integer("tutorId").notNull(),
      // user id
      planName: varchar("planName", { length: 200 }).notNull(),
      description: text("description"),
      crechePrice: integer("crechePrice"),
      // custom price for creche in cents (null = use standard)
      diariaPrice: integer("diariaPrice"),
    ...
    foodStock = pgTable("food_stock", {
      id: serial("id").primaryKey(),
      currentStockKg: integer("currentStockKg").default(0).notNull(),
      // in grams (e.g., 20000 for 20kg)
      lastUpdated: timestamp("lastUpdated").defaultNow().notNull()
    });
    foodMovements = pgTable("food_movements", {
      id: serial("id").primaryKey(),
      type: varchar("type", { length: 100 }).notNull(),
      amountKg: integer("amountKg").notNull(),
    ...
    tutorNotificationPreferences = pgTable("tutor_notification_preferences", {
      id: serial("id").primaryKey(),
      tutorId: integer("tutorId").notNull(),
    ...
    healthBehaviorLogs = pgTable("health_behavior_logs", {
      id: serial("id").primaryKey(),
      petId: integer("petId").notNull(),
    ...
    petFoodStock = pgTable("pet_food_stock", {
      id: serial("id").primaryKey(),
      petId: integer("petId").notNull(),
      brandName: varchar("brandName", { length: 200 }).notNull(),
      currentStock: integer("currentStock").notNull(),
      // current stock in grams
      dailyConsumption: integer("dailyConsumption").notNull(),
      // average daily consumption in grams
      alertThresholdDays: integer("alertThresholdDays").default(15).notNull(),
      // alert X days before running out
      lastPurchaseDate: timestamp("lastPurchaseDate"),
      lastPurchaseAmount: integer("lastPurchaseAmount"),
      // amount purchased in grams
      notes: text("notes"),
      createdAt: timestamp("createdAt").defaultNow().notNull(),
      updatedAt: timestamp("updatedAt").defaultNow().notNull()
    });
    medicationAutoScheduleRules = pgTable("medication_auto_schedule_rules", {
      id: serial("id").primaryKey(),
    ...
    wallPosts = pgTable("wall_posts", {
      id: serial("id").primaryKey(),
      petId: integer("petId"),
      // optional: post can be about a specific pet or general
      authorId: integer("authorId").notNull(),
      content: text("content"),
      mediaUrls: jsonb("mediaUrls").$type(),
      // array of photo/video URLs
      mediaKeys: jsonb("mediaKeys").$type(),
      // array of S3 keys
      postType: varchar("postType", { length: 100 }).default("text").notNull(),
      targetType: varchar("targetType", { length: 100 }).default("general").notNull(),
      // post visibility
    ...
    wallComments = pgTable("wall_comments", {
      id: serial("id").primaryKey(),
    ...
    wallReactions = pgTable("wall_reactions", {
      id: serial("id").primaryKey(),
      postId: integer("postId").notNull(),
      userId: integer("userId").notNull(),
    ...
    chatMessages = pgTable("chat_messages", {
      ...
      source: varchar("source", { length: 100 }).default("platform").notNull(),
      // message origin
...
__export(db_exports, {
  ...
  getPetsCountByStatus: () => getPetsCountByStatus,
  ...
  getTotalDailyFoodConsumption: () => getTotalDailyFoodConsumption,
  ...
  upsertUserFromSupabase: () => upsertUserFromSupabase
});
...
function detectConnectionType(url) {
  ...
  if (url.includes("pooler.supabase.com")) {
    if (url.includes(":5432")) return "session-mode";
    if (url.includes(":6543")) return "transaction-mode";
  }
  if (url.includes("db.") && url.includes(".supabase.co:5432")) return "direct";
  if (url.includes("db.") && url.includes(".supabase.co:6543")) return "transaction-mode";
  return "unknown";
}
async function getDb() {
  if (!_db && process.env.DATABASE_URL) {
    try {
      const connectionType = detectConnectionType(process.env.DATABASE_URL);
      const client = postgres(process.env.DATABASE_URL, {
        max: 20,
        // Increased from 10 - Maximum number of connections in the pool
        idle_timeout: 30,
        // Increased from 20 - Close idle connections after 30 seconds
        connect_timeout: 10
        // Connection timeout in seconds
        // For transaction mode, we might need to disable prepared statements
        // but postgres-js handles this automatically based on the connection
      });
      _db = drizzle(client);
      console.log(`[Database] Connected successfully (${connectionType})`);
      if (connectionType === "transaction-mode") {
        console.warn("[Database] Transaction mode detected - prepared statements may not work");
      } else if (connectionType === "direct") {
        console.warn("[Database] Direct connection detected - ensure IPv6 is available");
      }
    } catch (error) {
      console.error("[Database] Failed to connect:", error.message);
      console.error("[Database] Connection type:", detectConnectionType(process.env.DATABASE_URL || ""));
      _db = null;
    }
  }
  return _db;
  ...
  try {
    ...
    if (user.auth_id) {
      try {
        ...
        console.warn("[upsertUser] Error getting user by auth_id, continuing:", error.message);
      }
    }
    if (!existingUser && user.open_id) {
      try {
        ...
        console.warn("[upsertUser] Error getting user by open_id, continuing:", error.message);
      }
    }
    if (!existingUser && user.email) {
      try {
        ...
        console.warn("[upsertUser] Error getting user by email, continuing:", error.message);
      }
    }
    const updateSet = {};
    if (user.name !== void 0) updateSet.name = user.name;
    if (user.email !== void 0) updateSet.email = user.email;
    if (user.open_id !== void 0) updateSet.open_id = user.open_id;
    if (user.auth_id !== void 0) updateSet.auth_id = user.auth_id;
    if (user.login_method !== void 0) updateSet.login_method = user.login_method;
    if (user.role !== void 0) {
      updateSet.role = user.role;
    } else if (user.open_id === ENV.ownerOpenId) {
      updateSet.role = "admin";
    }
    updateSet.last_signed_in = user.last_signed_in || /* @__PURE__ */ new Date();
    updateSet.updated_at = /* @__PURE__ */ new Date();
    if (existingUser) {
      await db2.update(users).set(updateSet).where(eq(users.id, existingUser.id));
      console.log("[upsertUser] User updated, ID:", existingUser.id);
      ...
      console.log("[upsertUser] Inserting new user with values:", {
        auth_id: insertValues.auth_id,
        email: insertValues.email,
        name: insertValues.name,
        login_method: insertValues.login_method
      });
      try {
        const result = await db2.insert(users).values(insertValues).returning();
        const insertedId = result[0]?.id;
        console.log("[upsertUser] User inserted successfully, ID:", insertedId);
        if (!insertedId) {
          console.error("[upsertUser] No ID returned from insert");
          return null;
        }
        return insertedId;
      } catch (insertError) {
        console.error("[upsertUser] Error inserting user:", insertError.message);
        console.error("[upsertUser] Insert error stack:", insertError.stack);
        throw insertError;
      }
    }
  } catch (error) {
...
async function getUserByOpenId(openId) {
  ...
  const result = await db2.select().from(users).where(eq(users.open_id, openId)).limit(1);
  return result.length > 0 ? result[0] : void 0;
}
async function getUserByAuthId(authId) {
  const db2 = await getDb();
  if (!db2) {
    console.error("[getUserByAuthId] Database connection not available");
    return null;
  }
  try {
    console.log("[getUserByAuthId] Searching for auth_id:", authId);
    const result = await db2.select().from(users).where(eq(users.auth_id, authId)).limit(1);
    console.log("[getUserByAuthId] Query result:", result.length > 0 ? "Found" : "Not found");
    return result[0] || null;
  } catch (error) {
    console.error("[getUserByAuthId] Database error:", error.message);
    console.error("[getUserByAuthId] Error stack:", error.stack);
    console.error("[getUserByAuthId] Query params:", { authId });
    return null;
  }
}
async function upsertUserFromSupabase(supabaseUser) {
  ...
  await upsertUser({
    ...
    email: supabaseUser.email || void 0,
    name: supabaseUser.user_metadata?.name || void 0,
...
async function getAllUsers() {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(users).orderBy(desc(users.created_at));
}
...
async function updatePet(id, data) {
  ...
  if (data.approval_status) {
    ...
    cache2.delete("pets:pending:with-tutors");
  }
  if (data.food_amount || data.food_brand) {
    const { cache: cache2 } = await Promise.resolve().then(() => (init_cache(), cache_exports));
    cache2.delete("food:total-daily-consumption");
  }
}
...
async function getAllPets() {
  ...
  if (!db2) return [];
  try {
    return await db2.select().from(pets).orderBy(desc(pets.created_at));
  } catch (error) {
    console.warn("[getAllPets] Database error:", error.message);
    return [];
  }
}
async function getTotalDailyFoodConsumption() {
  const { cache: cache2 } = await Promise.resolve().then(() => (init_cache(), cache_exports));
  const cacheKey = "food:total-daily-consumption";
...
async function getPetsCountByStatus() {
  const { cache: cache2 } = await Promise.resolve().then(() => (init_cache(), cache_exports));
  const cacheKey = "pets:count-by-status";
  const cached = cache2.get(cacheKey);
  if (cached !== null) {
    return cached;
  }
  const db2 = await getDb();
  if (!db2) return { checkedIn: 0, checkedOut: 0, total: 0 };
  try {
    const result = await db2.select({
      status: pets.status,
      count: sql`COUNT(*)::int`
    }).from(pets).groupBy(pets.status);
    const checkedIn = result.find((r) => r.status === "checked-in")?.count || 0;
    const checkedOut = result.find((r) => r.status === "checked-out")?.count || 0;
    const total = result.reduce((sum, r) => sum + r.count, 0);
    const data = { checkedIn, checkedOut, total };
    cache2.set(cacheKey, data, 60 * 1e3);
    return data;
  } catch (error) {
    console.warn("[getPetsCountByStatus] Database error:", error.message);
    return { checkedIn: 0, checkedOut: 0, total: 0 };
  }
}
...
async function getPetsByApprovalStatus(approvalStatus) {
  const { cache: cache2 } = await Promise.resolve().then(() => (init_cache(), cache_exports));
  const cacheKey = `pets:approval:${approvalStatus}`;
  const cached = cache2.get(cacheKey);
  if (cached !== null) {
    return cached;
  }
  const db2 = await getDb();
  if (!db2) return [];
  const data = await db2.select().from(pets).where(eq(pets.approval_status, approvalStatus)).orderBy(desc(pets.created_at));
  cache2.set(cacheKey, data, 30 * 1e3);
  return data;
}
async function getPetsByTutorId(tutor_id) {
  const db2 = await getDb();
  if (!db2) return [];
  try {
    const result = await db2.select({ pet: pets }).from(petTutors).innerJoin(pets, eq(petTutors.pet_id, pets.id)).where(eq(petTutors.tutor_id, tutor_id));
    return result.map((r) => r.pet);
  } catch (error) {
    console.warn("[getPetsByTutorId] Database error:", error.message);
    return [];
  }
}
...
async function getUserByEmail(email) {
  const db2 = await getDb();
  if (!db2) {
    console.error("[getUserByEmail] Database connection not available");
    return null;
  }
  try {
    const normalizedEmail = email.toLowerCase().trim();
    console.log("[getUserByEmail] Searching for email:", normalizedEmail);
    const result = await db2.select().from(users).where(eq(users.email, normalizedEmail)).limit(1);
    console.log("[getUserByEmail] Query result:", result.length > 0 ? "Found" : "Not found");
    return result[0] || null;
  } catch (error) {
    console.error("[getUserByEmail] Database error:", error.message);
    console.error("[getUserByEmail] Error stack:", error.stack);
    console.error("[getUserByEmail] Query params:", { email: email.toLowerCase().trim() });
    return null;
  }
}
...
async function consumeCredit(pet_id2) {
  ...
  await db2.update(daycareCredits).set({ remaining_days: credit.remaining_days - 1 }).where(eq(daycareCredits.id, credit.id));
  return credit.id;
}
...
async function addVaccineToLibrary(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(vaccineLibrary).values({ ...data, isCommon: false }).returning();
  return result[0]?.id || 0;
}
...
async function getUpcomingVaccinations(daysAhead = 30) {
  const { cache: cache2 } = await Promise.resolve().then(() => (init_cache(), cache_exports));
  const cacheKey = `vaccinations:upcoming:${daysAhead}`;
  const cached = cache2.get(cacheKey);
  if (cached !== null) {
    return cached;
  }
  ...
  const data = await db2.select({
    ...
    vaccine: vaccineLibrary,
    pet: pets
  }).from(petVaccinations).innerJoin(vaccineLibrary, eq(petVaccinations.vaccine_id, vaccineLibrary.id)).innerJoin(pets, eq(petVaccinations.pet_id, pets.id)).where(and(
  ...
  cache2.set(cacheKey, data, 5 * 60 * 1e3);
  return data;
}
async function updatePetVaccination(id, data) {
  ...
  if (!db2) throw new Error("Database not available");
  const updates = {};
  if (data.nextDueDate !== void 0) updates.next_due_date = data.nextDueDate;
...
async function addMedicationToLibrary(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(medicationLibrary).values({ ...data, isCommon: false }).returning();
  return result[0]?.id || 0;
}
async function addPetMedication(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(petMedications).values(data).returning();
  return result[0]?.id || 0;
}
...
async function getActiveMedications(pet_id2) {
  ...
  const result = await db2.select({
    ...
    medicationInfo: medicationLibrary,
    pet: pets
  }).from(petMedications).innerJoin(medicationLibrary, eq(petMedications.medication_id, medicationLibrary.id)).innerJoin(pets, eq(petMedications.pet_id, pets.id)).where(and(eq(petMedications.pet_id, pet_id2), eq(petMedications.is_active, true))).orderBy(desc(petMedications.start_date));
  return result;
}
async function getAllActiveMedications() {
  ...
  const result = await db2.select({
    ...
    medicationInfo: medicationLibrary,
    pet: pets
  }).from(petMedications).innerJoin(medicationLibrary, eq(petMedications.medication_id, medicationLibrary.id)).innerJoin(pets, eq(petMedications.pet_id, pets.id)).where(eq(petMedications.is_active, true)).orderBy(desc(petMedications.start_date));
  return result;
}
...
async function getAllTransactions() {
  ...
  const result = await db2.select({
    ...
    petName: pets.name
  }).from(transactions).leftJoin(pets, eq(transactions.pet_id, pets.id)).orderBy(desc(transactions.transaction_date));
  return result.map((r) => ({
    ...r,
    amount: r.amount / 100
...
async function getAllLogs() {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(dailyLogs).orderBy(desc(dailyLogs.log_date)).limit(100);
}
async function addCalendarEvent(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(calendarEvents).values(data).returning();
  return result[0]?.id || 0;
}
...
async function getCalendarEvents(startDate, endDate) {
  ...
  return await db2.select({
    ...
    endDate: calendarEvents.end_date,
    eventType: calendarEvents.event_type,
    petId: calendarEvents.pet_id,
    location: calendarEvents.location,
    isAllDay: calendarEvents.is_all_day,
    checkInDate: calendarEvents.check_in_date,
    checkOutDate: calendarEvents.check_out_date,
    dailyCount: calendarEvents.daily_count,
    createdById: calendarEvents.created_by_id,
    createdAt: calendarEvents.created_at,
...
async function addDocument(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(documents).values(data).returning();
  return result[0]?.id || 0;
}
...
async function getAllDocuments(petId2, category) {
  ...
  }).from(documents).leftJoin(pets, eq(documents.pet_id, pets.id));
  const conditions = [];
  if (petId2) conditions.push(eq(documents.pet_id, petId2));
...
async function createNotification(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(notifications).values(data).returning();
  return result[0]?.id || 0;
}
async function getUserNotifications(user_id) {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(notifications).where(eq(notifications.user_id, user_id)).orderBy(desc(notifications.created_at));
}
async function markNotificationAsRead(id) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  await db2.update(notifications).set({ is_read: true }).where(eq(notifications.id, id));
}
async function markAllNotificationsAsRead(user_id) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  await db2.update(notifications).set({ is_read: true }).where(and(eq(notifications.user_id, user_id), eq(notifications.is_read, false)));
}
async function getUnreadNotificationCount(user_id) {
  const db2 = await getDb();
  if (!db2) return 0;
  const result = await db2.select().from(notifications).where(and(eq(notifications.user_id, user_id), eq(notifications.is_read, false)));
  return result.length;
}
async function getPendingNotifications() {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(notifications).where(eq(notifications.is_sent, false)).orderBy(notifications.created_at);
}
...
async function getTransactionsByDateRange(startDate, endDate) {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(transactions).where(and(gte(transactions.transaction_date, startDate), lte(transactions.transaction_date, endDate))).orderBy(desc(transactions.transaction_date));
}
async function getFinancialSummary(startDate, endDate) {
  const { cache: cache2 } = await Promise.resolve().then(() => (init_cache(), cache_exports));
  const cacheKey = `financial:summary:${startDate.getTime()}:${endDate.getTime()}`;
  const cached = cache2.get(cacheKey);
  if (cached !== null) {
    return cached;
  }
  ...
  }).from(transactions).where(and(gte(transactions.transaction_date, startDate), lte(transactions.transaction_date, endDate))).groupBy(transactions.type);
...
async function getVaccinesDueSoon(daysAhead = 30) {
  ...
  return await db2.select({
    ...
    pet: pets,
    vaccine: vaccineLibrary
  }).from(petVaccinations).innerJoin(pets, eq(petVaccinations.pet_id, pets.id)).innerJoin(vaccineLibrary, eq(petVaccinations.vaccine_id, vaccineLibrary.id)).where(
    and(
      lte(petVaccinations.next_due_date, futureDate),
      gte(petVaccinations.next_due_date, /* @__PURE__ */ new Date())
    )
  ).orderBy(petVaccinations.next_due_date);
}
...
async function addPhotoComment(photoId, user_id, comment) {
  const db2 = await getDb();
  await db2.insert(photoComments).values({ photoId, user_id, comment });
...
async function addPhotoReaction(photoId, user_id, reactionType) {
  ...
  if (existing.length > 0) {
    ...
    await db2.insert(photoReactions).values({ photoId, user_id, reactionType });
...
async function listAllUsers() {
  const db2 = await getDb();
  return db2.select().from(users).orderBy(users.created_at);
}
async function getUserById(user_id) {
  const db2 = await getDb();
  const result = await db2.select().from(users).where(eq(users.id, user_id)).limit(1);
  return result[0];
}
async function updateUser(user_id, data) {
  const db2 = await getDb();
  await db2.update(users).set(data).where(eq(users.id, user_id));
}
async function updateUserRole(user_id, role) {
  const db2 = await getDb();
  await db2.update(users).set({ role }).where(eq(users.id, user_id));
}
async function deleteUser(user_id) {
  const db2 = await getDb();
  await db2.delete(users).where(eq(users.id, user_id));
}
...
async function addPreventiveToLibrary(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(preventiveLibrary).values(data).returning();
  return result[0]?.id;
}
...
async function createWhatsAppTemplate(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(whatsappTemplates).values(data).returning();
  return result[0]?.id;
}
...
async function createWhatsAppGroup(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(whatsappGroups).values(data).returning();
  return result[0]?.id;
}
...
async function createWhatsAppAutomation(data) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  const result = await db2.insert(whatsappAutomations).values(data).returning();
  return result[0]?.id;
}
...
async function getUpcomingVaccinationReminders(daysAhead = 7) {
  ...
  }).from(petVaccinations).innerJoin(pets, eq(petVaccinations.pet_id, pets.id)).innerJoin(vaccineLibrary, eq(petVaccinations.vaccine_id, vaccineLibrary.id)).where(and(
    gte(petVaccinations.next_due_date, today),
    lte(petVaccinations.next_due_date, futureDate)
  )).orderBy(petVaccinations.next_due_date);
  return results;
...
async function getEndingMedicationReminders(daysAhead = 7) {
  ...
  }).from(petMedications).innerJoin(pets, eq(petMedications.pet_id, pets.id)).innerJoin(medicationLibrary, eq(petMedications.medication_id, medicationLibrary.id)).where(and(
...
async function getActiveMedicationsCount() {
  ...
  const activeMeds = await db2.select({
    ...
    medicationName: medicationLibrary.name,
    endDate: petMedications.end_date
  }).from(petMedications).innerJoin(pets, eq(petMedications.pet_id, pets.id)).innerJoin(medicationLibrary, eq(petMedications.medication_id, medicationLibrary.id)).where(gte(petMedications.end_date, today));
...
async function getOverdueTreatments() {
  ...
  }).from(petVaccinations).innerJoin(pets, eq(petVaccinations.pet_id, pets.id)).innerJoin(vaccineLibrary, eq(petVaccinations.vaccine_id, vaccineLibrary.id)).where(lt(petVaccinations.next_due_date, today)).orderBy(petVaccinations.next_due_date);
...
async function getHealthCalendarEvents(startDate, endDate) {
  ...
  const vaccinations = await db2.select({
    ...
    next_due_date: petVaccinations.next_due_date,
    notes: petVaccinations.notes
  }).from(petVaccinations).leftJoin(pets, eq(petVaccinations.pet_id, pets.id)).leftJoin(vaccineLibrary, eq(petVaccinations.vaccine_id, vaccineLibrary.id)).where(
    and(
      startDate ? gte(petVaccinations.next_due_date, startDate) : void 0,
      endDate ? lte(petVaccinations.next_due_date, endDate) : void 0
    )
  ).orderBy(petVaccinations.next_due_date);
  ...
  const medications = await db2.select({
    ...
    endDate: petMedications.end_date,
    notes: petMedications.notes
  }).from(petMedications).leftJoin(pets, eq(petMedications.pet_id, pets.id)).leftJoin(medicationLibrary, eq(petMedications.medication_id, medicationLibrary.id)).where(
    and(
      startDate ? gte(petMedications.end_date, startDate) : void 0,
      endDate ? lte(petMedications.end_date, endDate) : void 0
    )
  ).orderBy(petMedications.end_date);
...
async function getTutorById(tutor_id) {
  const db2 = await getDb();
  if (!db2) return null;
  const tutorResult = await db2.select().from(users).where(and(eq(users.id, tutor_id), eq(users.role, "user"))).limit(1);
  ...
  }).from(petTutors).leftJoin(pets, eq(petTutors.pet_id, pets.id)).where(eq(petTutors.tutor_id, tutorId)).orderBy(desc(petTutors.is_primary), pets.name);
  return {
    ...tutor,
    linkedPets
  };
}
async function updateTutor(tutor_id, data) {
  const db2 = await getDb();
  if (!db2) return null;
...
async function getAllCalendarEvents(startDate, endDate) {
  ...
  const paymentsData = await db2.select({
    ...
    transactionDate: transactions.transaction_date,
    type: transactions.type
  }).from(transactions).leftJoin(pets, eq(transactions.pet_id, pets.id)).where(
    and(
...
async function getCreditConsumption(startDate, endDate) {
  ...
  const usageData = await db2.select({
    ...
    petName: pets.name,
    count: sql`COUNT(*)`
  }).from(daycareUsage).leftJoin(pets, eq(daycareUsage.pet_id, pets.id)).where(
    and(
...
async function markMessagesAsRead(conversationId, user_id) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
...
Pr\xF3xima aplica\xE7\xE3o: ${next_due_date.toLocaleDateString("pt-BR")}`;
  const eventId = await addCalendarEvent({
...
async function loginWithEmail(email, password) {
  try {
    ...
    if (error) {
      ...
      if (error.message.includes("Email not confirmed") || error.message.includes("email_not_confirmed") || error.status === 400 && error.message.toLowerCase().includes("confirm")) {
      ...
      if (error) {
        if (error.message.includes("Invalid login credentials") || error.message.includes("User not found") || error.message.includes("Invalid login")) {
          if (localUser && !localUser.auth_id) {
    ...
    if (!user) {
      ...
      user = {
        id: 0,
        // ID temporário - será atualizado quando o banco estiver disponível
        auth_id: data.user.id,
        email: data.user.email,
        name: data.user.email.split("@")[0],
        role: "user",
        ...
        created_at: /* @__PURE__ */ new Date(),
        updated_at: /* @__PURE__ */ new Date(),
        last_signed_in: /* @__PURE__ */ new Date()
      };
    } else {
      const db2 = await getDb();
      if (db2 && user.id) {
        try {
          await db2.update(users).set({ last_signed_in: /* @__PURE__ */ new Date() }).where(eq2(users.id, user.id));
        } catch (updateError) {
...
async function createUserInSupabase(email, password, name, emailVerified = true) {
  try {
...
async function changePassword2(userId2, oldPassword, newPassword) {
  try {
    const db2 = await getDb();
    if (!db2) throw new Error("Database not available");
    const userResults = await db2.select().from(users).where(eq2(users.id, userId2)).limit(1);
...
async function resetPassword2(token, newPassword) {
  ...
  const userResults = await db2.select().from(users).where(eq2(users.id, resetToken.user_id)).limit(1);
  const user = userResults.length > 0 ? userResults[0] : null;
  if (!user || !user.auth_id) {
    throw new Error("User not found or not using Supabase Auth");
  }
  ...
  await db2.update(passwordResetTokens).set({ used: true }).where(eq2(passwordResetTokens.id, resetToken.id));
  return { success: true };
}
...
async function verifyEmail(token) {
  ...
  await db2.update(users).set({ email_verified: true }).where(eq2(users.id, verificationToken.user_id));
  await db2.update(emailVerificationTokens).set({ used: true }).where(eq2(emailVerificationTokens.id, verificationToken.id));
  return { success: true };
}
...
async function getResourceHistory(resourceType, resourceId) {
  const { eq: eq15, and: and9 } = await import("drizzle-orm");
  return database.select().from(changeHistory).where(
    and9(
      eq15(changeHistory.resourceType, resourceType),
      eq15(changeHistory.resourceId, resourceId)
    )
  ).orderBy(changeHistory.createdAt);
}
async function getPetHistory(petId2) {
  const { eq: eq15 } = await import("drizzle-orm");
  return database.select().from(changeHistory).where(eq15(changeHistory.petId, petId2)).orderBy(changeHistory.createdAt);
}
...
async function getChangesByUser(userId2) {
  const { eq: eq15 } = await import("drizzle-orm");
  return database.select().from(changeHistory).where(eq15(changeHistory.changedBy, userId2)).orderBy(changeHistory.createdAt);
}
...
async function getActivityByDay(days = 30) {
  const { sql: sql5, desc: desc5 } = await import("drizzle-orm");
  const startDate = /* @__PURE__ */ new Date();
  startDate.setDate(startDate.getDate() - days);
  const allChanges = await database.select().from(changeHistory).where(sql5`${changeHistory.createdAt} >= ${startDate}`).orderBy(desc5(changeHistory.createdAt));
...
var init_changeTracker = __esm({
  "server/changeTracker.ts"() {
    "use strict";
    init_schema();
    connection = postgres2(process.env.DATABASE_URL);
...
async function notifyCheckIn(petId2) {
  try {
    const pet = await getPetById(petId2);
    if (!pet) return;
    const tutors = await getPetTutors(petId2);
...
async function notifyCheckOut(petId2) {
  try {
    const pet = await getPetById(petId2);
    if (!pet) return;
    const tutors = await getPetTutors(petId2);
...
async function notifyDailyReport(petId2, reportSummary) {
  try {
    const pet = await getPetById(petId2);
    if (!pet) return;
    const tutors = await getPetTutors(petId2);
...
async function notifyCalendarEvent(petId2, eventTitle, eventDate) {
  try {
    const pet = await getPetById(petId2);
    if (!pet) return;
    const tutors = await getPetTutors(petId2);
...
async function notifyMedicationReminder(petId2, medicationName, dosage, time) {
  try {
    const pet = await getPetById(petId2);
    if (!pet) return;
    const tutors = await getPetTutors(petId2);
...
async function notifyLowCredits(petId2, remainingCredits) {
  try {
    const pet = await getPetById(petId2);
    if (!pet) return;
    const tutors = await getPetTutors(petId2);
...
async function getOccupancyReport(startDate, endDate) {
  ...
  const { between, and: and9, isNotNull: isNotNull3, sql: sql5 } = await import("drizzle-orm");
  const MAX_CAPACITY = 20;
  const checkIns = await dbInstance.select({
    date: sql5`DATE(${daycareUsage2.check_in_time})`,
    count: sql5`COUNT(*)`,
    revenue: sql5`SUM(COALESCE(${daycareUsage2.amount_cents}, 0))`
  }).from(daycareUsage2).where(
    and9(
      between(daycareUsage2.check_in_time, startDate, endDate),
      isNotNull3(daycareUsage2.check_in_time)
    )
  ).groupBy(sql5`DATE(${daycareUsage2.check_in_time})`).orderBy(sql5`DATE(${daycareUsage2.check_in_time})`);
...
async function getAdminLogs(filters) {
  ...
  if (filters?.adminId) {
    query = query.where(eq3(adminLogs.adminId, filters.adminId));
  }
  if (filters?.action) {
    query = query.where(eq3(adminLogs.action, filters.action));
  }
...
async function handleCheckoutCompleted(session) {
  ...
  if (session.customer) {
    await db2.update(users).set({ stripeCustomerId: session.customer }).where(eq4(users.id, parseInt(userId2)));
  }
...
async function handlePaymentSucceeded(paymentIntent) {
  ...
  if (productType === "credits" && creditsToAdd > 0) {
    const userPets = await db2.select().from(pets).where(eq4(pets.id, parseInt(userId2)));
    if (userPets.length > 0) {
      const creditsPerPet = Math.floor(creditsToAdd / userPets.length);
      for (const pet of userPets) {
        await db2.update(pets).set({ credits: (pet.credits || 0) + creditsPerPet }).where(eq4(pets.id, pet.id));
      }
    }
  }
...
async function updateMessageStatus(whatsappMessageId, status, errorMessage) {
  ...
  try {
    ...
    await db2.update(whatsappMessages).set(updateData).where(eq5(whatsappMessages.whatsappMessageId, whatsappMessageId));
  } catch (error) {
...
__export(notification_exports, {
  notifyOwner: () => notifyOwner2
});
import { TRPCError } from "@trpc/server";
async function notifyOwner2(payload) {
...
async function createBookingRequest(data) {
  ...
  const [request] = await db2.select().from(bookingRequests).where(eq6(bookingRequests.id, insertId)).limit(1);
  return request;
}
async function getTutorBookingRequests(tutorId2) {
  const db2 = await getDb();
  ...
  }).from(bookingRequests).leftJoin(pets, eq6(bookingRequests.petId, pets.id)).where(eq6(bookingRequests.tutorId, tutorId2)).orderBy(sql2`${bookingRequests.createdAt} DESC`);
  return requests;
}
async function getPendingBookingRequests() {
  ...
  const requests = await db2.select({
    ...
    notes: bookingRequests.notes,
    createdAt: bookingRequests.createdAt
  }).from(bookingRequests).leftJoin(pets, eq6(bookingRequests.petId, pets.id)).leftJoin(users, eq6(bookingRequests.tutorId, users.open_id)).where(eq6(bookingRequests.status, "pending")).orderBy(sql2`${bookingRequests.createdAt} ASC`);
  return requests;
}
async function getAllBookingRequests(status) {
  ...
  let query = db2.select({
    ...
    approvedAt: bookingRequests.approvedAt,
    createdAt: bookingRequests.createdAt
  }).from(bookingRequests).leftJoin(pets, eq6(bookingRequests.petId, pets.id)).leftJoin(users, eq6(bookingRequests.tutorId, users.open_id));
  if (status) {
    const requests2 = await query.where(eq6(bookingRequests.status, status)).orderBy(sql2`${bookingRequests.createdAt} DESC`);
    return requests2;
  }
...
async function approveBookingRequest(requestId, adminId, adminNotes) {
  const db2 = await getDb();
  const [request] = await db2.select().from(bookingRequests).where(eq6(bookingRequests.id, requestId));
  if (!request) {
  ...
  const [pet] = await db2.select().from(pets).where(eq6(pets.id, request.petId));
  if (!pet) {
    throw new Error("Pet not found");
  }
  ...
  const [tutor] = await db2.select({ id: users.id }).from(users).where(eq6(users.open_id, request.tutorId));
  if (!tutor) {
  ...
  }).where(eq6(pets.id, request.petId));
  await db2.update(bookingRequests).set({
    ...
    adminNotes,
    updatedAt: /* @__PURE__ */ new Date()
  }).where(eq6(bookingRequests.id, requestId));
...
async function cancelBookingRequest(requestId, tutorId2) {
  const db2 = await getDb();
  const [request] = await db2.select().from(bookingRequests).where(eq6(bookingRequests.id, requestId));
  if (!request) {
    throw new Error("Booking request not found");
  }
  if (request.tutorId !== tutorId2) {
...
function initializeWebSocket(httpServer) {
  ...
  io.use(async (socket, next) => {
    ...
    try {
      ...
      const [user] = await db.select().from(users).where(eq7(users.open_id, session.openId)).limit(1);
      if (!user) {
...
var init_websocket = __esm({
  "server/_core/websocket.ts"() {
    ...
    init_sdk();
    init_schema();
    connection2 = postgres3(process.env.DATABASE_URL);
...
async function getPrimaryTutorForPet(petId2) {
  ...
  const tutorRelations = await database2.select({
    tutorId: petTutors3.tutor_id,
    isPrimary: petTutors3.is_primary
  }).from(petTutors3).where(eq15(petTutors3.pet_id, petId2));
  ...
  const tutors = await database2.select({
    name: users2.name,
    phone: users2.phone
  }).from(users2).where(eq15(users2.id, primaryRelation.tutorId));
...
async function sendAllHealthReminders(daysAhead = 7) {
  ...
  try {
    ...
    for (const vaccine of vaccines) {
      const pet = await getPetById(vaccine.pet_id);
      if (!pet) continue;
      ...
      const daysUntilDue = Math.ceil((vaccine.next_due_date.getTime() - Date.now()) / (1e3 * 60 * 60 * 24));
    ...
    for (const medication of medications) {
      const pet = await getPetById(medication.pet_id);
      if (!pet) continue;
    ...
    for (const treatment of flea) {
      const pet = await getPetById(treatment.pet_id);
      if (!pet) continue;
      ...
      const daysUntilDue = Math.ceil((treatment.next_due_date.getTime() - Date.now()) / (1e3 * 60 * 60 * 24));
      remindersByOwner.get(tutor.phone).reminders.push({
        type: "flea",
        petName: pet.name,
...
async function getServicePrices() {
  const db2 = await getDb();
  return db2.select().from(servicePrices).where(eq8(servicePrices.isActive, true));
}
async function getServicePrice(serviceType) {
  const db2 = await getDb();
  const [price] = await db2.select().from(servicePrices).where(
    and4(
      eq8(servicePrices.serviceType, serviceType),
      eq8(servicePrices.isActive, true)
    )
  );
  return price;
}
async function getCustomPricingPlan(tutorId2) {
  const db2 = await getDb();
  const [plan] = await db2.select().from(customPricingPlans).where(
    and4(
      eq8(customPricingPlans.tutorId, tutorId2),
      eq8(customPricingPlans.isActive, true)
    )
  ).orderBy(customPricingPlans.createdAt);
  return plan;
}
async function getEffectivePrice(tutorId2, serviceType) {
  const customPlan = await getCustomPricingPlan(tutorId2);
  if (customPlan) {
...
async function updateCustomPricingPlan(id, data) {
  const db2 = await getDb();
  await db2.update(customPricingPlans).set(data).where(eq8(customPricingPlans.id, id));
}
async function deactivateCustomPricingPlan(id) {
  const db2 = await getDb();
  await db2.update(customPricingPlans).set({ isActive: false }).where(eq8(customPricingPlans.id, id));
}
...
async function getFoodStats() {
  ...
  cache.set(cacheKey, data, 60 * 1e3);
  return data;
}
async function addStock(amountKg, notes, createdBy) {
  ...
  if (stock.id > 0) {
    ...
    }).where(eq9(foodStock.id, stock.id));
  } else {
...
async function sendWhatsAppMessage(params) {
  ...
  const [config] = await db2.select().from((await Promise.resolve().then(() => (init_schema(), schema_exports))).whatsappConfig).where(
    eq10((await Promise.resolve().then(() => (init_schema(), schema_exports))).whatsappConfig.isActive, true)
  );
  ...
  try {
    ...
    for (const message of value.messages) {
      ...
      const [existing] = await db2.select().from(chatMessages).where(
        eq10(chatMessages.whatsappMessageId, whatsappMessageId)
      );
      if (existing) {
        continue;
      }
      const [user] = await db2.select().from(users).where(
        eq10(users.phone, fromPhone)
      );
      if (!user) {
      ...
      await db2.update(conversations).set({ lastMessageAt: new Date(timestamp2) }).where(eq10(conversations.id, conversation.id));
    }
...
async function syncMessageToWhatsApp(messageId) {
  ...
  await db2.update(chatMessages).set({ whatsappMessageId }).where(eq10(chatMessages.id, messageId));
...
var systemRouter = router({
  ...
  notifyOwner: adminProcedure.input(
    ...
    const delivered = await notifyOwner2(input);
    return {
      success: delivered
    };
  })
...
init_schema();
init_whatsappService();
import { and as and6, eq as eq11, gte as gte3, lte as lte3, isNotNull as isNotNull2 } from "drizzle-orm";
async function checkUpcomingVaccines() {
  ...
  try {
    ...
    const upcomingVaccines = await db2.select({
      ...
      pet: pets,
      vaccine: vaccineLibrary
    }).from(petVaccinations).innerJoin(pets, eq11(petVaccinations.pet_id, pets.id)).innerJoin(vaccineLibrary, eq11(petVaccinations.vaccine_id, vaccineLibrary.id)).where(
      and6(
        isNotNull2(petVaccinations.next_due_date),
        gte3(petVaccinations.next_due_date, today),
        lte3(petVaccinations.next_due_date, sevenDaysFromNow)
      )
    );
    ...
    for (const { vaccination, pet, vaccine } of upcomingVaccines) {
      try {
        ...
        }).from(petTutors).innerJoin(users, eq11(petTutors.tutor_id, users.id)).where(eq11(petTutors.pet_id, pet.id)).orderBy(petTutors.is_primary);
...
import { or as or3, like as like2 } from "drizzle-orm";
  ...
  global: protectedProcedure.input(z2.object({
    ...
    const documentsResults = await db2.select().from(documents).where(
      or3(
        ...
        like2(documents.category, searchTerm)
      )
    ).limit(5);
    return {
      pets: petsResults,
...
var appRouter = router({
  ...
  auth: router({
    ...
    login: publicProcedure.input(z3.object({
      ...
      const { sdk: sdk2 } = await Promise.resolve().then(() => (init_sdk(), sdk_exports));
      const { loginWithEmail: loginWithEmail2 } = await Promise.resolve().then(() => (init_emailAuth(), emailAuth_exports));
      console.log("[login route] Attempting login for:", input.email);
      const user = await loginWithEmail2(input.email, input.password);
      if (!user) {
      ...
      const userName = user.name || user.email || "User";
      const sessionToken = await sdk2.createSessionToken(userIdForSession, {
        name: userName,
        expiresInMs: 365 * 24 * 60 * 60 * 1e3
        // 1 year
      });
    ...
    verifyEmail: publicProcedure.input(z3.object({
      ...
      const { verifyEmail: verifyEmailToken } = await Promise.resolve().then(() => (init_emailAuth(), emailAuth_exports));
  ...
  pets: router({
    ...
    create: protectedProcedure.input(z3.object({
      ...
      birth_date: z3.string().optional(),
      food_brand: z3.string().optional(),
    ...
    updateAdmin: adminProcedure2.input(z3.object({
      ...
      age: z3.string().optional(),
      weight: z3.number().optional(),
      birth_date: z3.date().optional(),
      photo_url: z3.string().optional(),
      photo_key: z3.string().optional(),
      food_brand: z3.string().optional(),
      food_amount: z3.number().optional(),
      notes: z3.string().optional()
    })).mutation(async ({ input, ctx }) => {
      ...
      if (data.weight || data.food_brand || data.food_amount) {
        ...
        if (data.food_brand) changes.push(`Ra\xE7\xE3o: ${data.food_brand}`);
    ...
    getFrequencyStats: protectedProcedure.input(z3.object({ petId: z3.number() })).query(async ({ input }) => {
      ...
      daycareLog.forEach((log) => {
        const date2 = new Date(log.log_date);
    ...
    listPending: adminProcedure2.query(async () => {
      ...
      try {
        const result = await dbInstance.select({
          ...
          tutor: users,
          isPrimary: petTutors.is_primary
        }).from(pets).leftJoin(petTutors, eq14(pets.id, petTutors.pet_id)).leftJoin(users, eq14(petTutors.tutor_id, users.id)).where(eq14(pets.approval_status, "pending")).orderBy(desc4(pets.created_at));
        const petMap = /* @__PURE__ */ new Map();
        for (const row of result) {
          const petId2 = row.pet.id;
          if (!petMap.has(petId2)) {
            petMap.set(petId2, {
              ...row.pet,
              tutorName: null,
              tutorId: null
          ...
          const current = petMap.get(petId2);
        ...
        const petsList = await getPetsByApprovalStatus("pending");
        const enrichedPets = await Promise.all(
          petsList.map(async (pet) => {
            const tutors = await getPetTutors(pet.id);
            const primaryTutor = tutors.find((t2) => t2.is_primary) || tutors[0];
            return {
              ...pet,
        ...
        cache2.set(cacheKey, enrichedPets, 30 * 1e3);
        return enrichedPets;
      }
    }),
  ...
  vaccines: router({
    ...
    addVaccination: protectedProcedure.input(z3.object({
      ...
      const id = await addPetVaccination({
        pet_id: input.petId,
    ...
    update: protectedProcedure.input(z3.object({
      ...
      const { id, petId: petId2, ...data } = input;
  ...
  medications: router({
    ...
    scheduleNext: protectedProcedure.input(z3.object({
      ...
      const nextDate = calculateNextDose2(/* @__PURE__ */ new Date(), {
        ...
        customInterval: med.custom_interval || void 0,
        weekDays: med.week_days ? JSON.parse(med.week_days) : void 0,
        monthDays: med.month_days ? JSON.parse(med.month_days) : void 0
      });
      if (!nextDate) {
      ...
      if (med.dosage_progression && med.dosage_progression !== "stable") {
        currentDosage = calculateProgressiveDosage2(med.dosage, {
          ...
          targetDosage: med.target_dosage || void 0,
  ...
  logs: router({
    ...
    listMine: protectedProcedure.query(async ({ ctx }) => {
      ...
      const allLogs = [];
      for (const petId2 of petIds) {
        const logs = await getPetLogs(petId2);
        allLogs.push(...logs);
      }
      return allLogs.sort(
  ...
  calendar: router({
    ...
    update: protectedProcedure.input(z3.object({
      ...
      const { id, petId: petId2, eventDate, endDate, eventType, isAllDay, ...rest } = input;
      const data = { ...rest };
      if (eventDate !== void 0) data.event_date = eventDate;
      if (endDate !== void 0) data.end_date = endDate;
      if (eventType !== void 0) data.event_type = eventType;
      if (isAllDay !== void 0) data.is_all_day = isAllDay;
      await updateCalendarEvent(id, data);
      if (petId2 && Object.keys(data).length > 0) {
        await logChange({
          resourceType: "calendar",
          resourceId: id,
          petId: petId2,
  ...
  dashboard: router({
    stats: adminProcedure2.query(async () => {
      ...
      try {
        const petsCount = await getPetsCountByStatus();
        const today = /* @__PURE__ */ new Date();
        ...
        const [monthlyFinances, upcomingVaccines] = await Promise.allSettled([
          ...
          getUpcomingVaccinations(30)
        ]);
        const finances = monthlyFinances.status === "fulfilled" ? monthlyFinances.value : { credits: 0, debits: 0, balance: 0 };
        const vaccines = upcomingVaccines.status === "fulfilled" ? upcomingVaccines.value : [];
        const data = {
    ...
    checkUpcomingVaccines: adminProcedure2.input(z3.object({ daysAhead: z3.number().default(30) })).mutation(async ({ input }) => {
      ...
      if (vaccinesDue.length > 0) {
        const vaccineList = vaccinesDue.map(
          (v) => `${v.pet.name} - ${v.vaccine.name} (${new Date(v.vaccination.next_due_date).toLocaleDateString("pt-BR")})`
        ).join(", ");
        await notifyOwner2({
...
Mantenha as respostas concisas (m\xE1ximo 3 par\xE1grafos) e pr\xE1ticas.`;
  ...
  userManagement: router({
     ...
     */
    createInSupabase: adminProcedure2.input(z3.object({
      email: z3.string().email(),
      password: z3.string().min(6),
      name: z3.string().min(1),
      emailVerified: z3.boolean().optional().default(true)
    })).mutation(async ({ input, ctx }) => {
      const { createUserInSupabase: createUserInSupabase2 } = await Promise.resolve().then(() => (init_emailAuth(), emailAuth_exports));
      const user = await createUserInSupabase2(
        input.email,
        input.password,
        input.name,
        input.emailVerified
...
`;
      ...
      const { notifyOwner: notifyOwner3 } = await Promise.resolve().then(() => (init_notification(), notification_exports));
      const sent = await notifyOwner3({
  ...
  healthCalendar: router({
    ...
    createVaccination: adminProcedure2.input(z3.object({
      ...
      return await addPetVaccination({
        pet_id: input.petId,
        vaccine_id: input.vaccineId,
        application_date: input.applicationDate,
        next_due_date: input.nextDueDate,
    ...
    createTransaction: adminProcedure2.input(z3.object({
      ...
      const { petId: petId2, transactionDate, ...rest } = input;
      const data = {
        ...rest,
        pet_id: petId2,
    ...
    deleteBooking: adminProcedure2.input(z3.object({ id: z3.number() })).mutation(async ({ input }) => {
      ...
      const dbInstance = await getDb2();
      if (!dbInstance) throw new Error("Database not available");
      await dbInstance.delete(bookings2).where(eq15(bookings2.id, input.id));
      return true;
    }),
    /**
    ...
    updateBooking: adminProcedure2.input(z3.object({
      ...
      await dbInstance.update(bookings2).set({
        ...
        numberOfDays,
        notes: input.notes
      }).where(eq15(bookings2.id, input.id));
      return true;
    }),
    /**
     * Update transaction
    ...
    updateTransaction: adminProcedure2.input(z3.object({
      ...
      }).where(eq15(transactions2.id, input.id));
      return true;
    })
  }),
  // ==================== TUTORS MANAGEMENT ====================
  tutors: router({
    ...
    update: adminProcedure2.input(z3.object({
      ...
      const { tutorId: tutorId2, ...data } = input;
      return await updateTutor(tutorId2, data);
    }),
    /**
     * Link pet to tutor
     */
  ...
  bookingRequests: router({
    ...
    myRequests: protectedProcedure.query(async ({ ctx }) => {
      ...
      return await getTutorBookingRequests2(ctx.user.open_id || ctx.user.email || "");
    }),
    /**
    ...
    cancel: protectedProcedure.input(z3.object({ id: z3.number() })).mutation(async ({ input, ctx }) => {
      ...
      return await cancelBookingRequest2(input.id, ctx.user.open_id || ctx.user.email || "");
    }),
    /**
    ...
    approve: adminProcedure2.input(z3.object({
      ...
      const [request] = await db2.select().from(bookingRequests2).where(eq15(bookingRequests2.id, input.id));
      const result = await approveBookingRequest2(input.id, ctx.user.open_id || ctx.user.email || "", input.adminNotes);
      if (request) {
    ...
    reject: adminProcedure2.input(z3.object({
      ...
      const db2 = await getDb2();
      const [request] = await db2.select().from(bookingRequests2).where(eq15(bookingRequests2.id, input.id));
      const result = await rejectBookingRequest2(input.id, ctx.user.open_id || ctx.user.email || "", input.adminNotes);
      if (request) {
        const { notifyUser: notifyUser2 } = await Promise.resolve().then(() => (init_websocket(), websocket_exports));
        notifyUser2(request.tutorId, {
  ...
  food: router({
     ...
     */
    getTotalDailyConsumption: adminProcedure2.query(async () => {
      return await getTotalDailyFoodConsumption();
    }),
    /**
     * Add stock
     */
...
function getSupabaseAdmin() {
  if (!supabaseAdmin) {
    ...
    supabaseAdmin = createClient2(supabaseUrl, supabaseServiceKey, {
      auth: {
        autoRefreshToken: false,
        persistSession: false
      }
