...
async function upsertUser(user) {
  ...
  try {
    ...
    if (existingUser) {
      ...
      await db2.insert(users).values(insertValues);
    }
  } catch (error) {
...
async function getPetsByApprovalStatus(approvalStatus) {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(pets).where(eq(pets.approval_status, approvalStatus)).orderBy(desc(pets.created_at));
}
...
async function getCalendarEvents(startDate, endDate) {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(calendarEvents).where(and(gte(calendarEvents.event_date, startDate), lte(calendarEvents.event_date, endDate))).orderBy(calendarEvents.event_date);
}
...
async function getUserNotifications(user_id2) {
  const db2 = await getDb();
  if (!db2) return [];
  return await db2.select().from(notifications).where(eq(notifications.user_id, user_id2)).orderBy(desc(notifications.created_at));
}
...
async function markAllNotificationsAsRead(user_id2) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
  await db2.update(notifications).set({ is_read: true }).where(and(eq(notifications.user_id, user_id2), eq(notifications.is_read, false)));
}
async function getUnreadNotificationCount(user_id2) {
  const db2 = await getDb();
  if (!db2) return 0;
  const result = await db2.select().from(notifications).where(and(eq(notifications.user_id, user_id2), eq(notifications.is_read, false)));
...
async function getUserActiveSubscription(user_id2) {
  const db2 = await getDb();
...
async function getUserSubscriptionHistory(user_id2) {
  const db2 = await getDb();
  return db2.select().from(subscriptions).where(eq(subscriptions.user_id, user_id2)).orderBy(desc(subscriptions.created_at));
}
...
async function addPhotoComment(photoId, user_id2, comment) {
  const db2 = await getDb();
  await db2.insert(photoComments).values({ photoId, user_id: user_id2, comment });
...
async function addPhotoReaction(photoId, user_id2, reactionType) {
  const db2 = await getDb();
  const existing = await db2.select().from(photoReactions).where(and(eq(photoReactions.photoId, photoId), eq(photoReactions.user_id, user_id2))).limit(1);
  if (existing.length > 0) {
    await db2.update(photoReactions).set({ reactionType }).where(eq(photoReactions.id, existing[0].id));
    return existing[0];
  } else {
    await db2.insert(photoReactions).values({ photoId, user_id: user_id2, reactionType });
    const [newReaction] = await db2.select().from(photoReactions).where(eq(photoReactions.id, sql`LAST_INSERT_ID()`)).limit(1);
    return newReaction;
  }
}
async function removePhotoReaction(photoId, user_id2) {
  const db2 = await getDb();
...
async function listBookings(filters) {
  ...
  if (filters?.petId) {
    query = query.where(eq(bookings.pet_id, filters.petId));
  }
  if (filters?.tutorId) {
    query = query.where(eq(bookings.tutor_id, filters.tutorId));
  }
  if (filters?.status) {
...
async function updateBookingStatus(id, status) {
  const db2 = await getDb();
  await db2.update(bookings).set({ status, updated_at: /* @__PURE__ */ new Date() }).where(eq(bookings.id, id));
}
...
async function getUserById(user_id2) {
  const db2 = await getDb();
  const result = await db2.select().from(users).where(eq(users.id, user_id2)).limit(1);
  return result[0];
}
async function updateUser(user_id2, data) {
  const db2 = await getDb();
  await db2.update(users).set(data).where(eq(users.id, user_id2));
}
async function updateUserRole(user_id2, role) {
  const db2 = await getDb();
  await db2.update(users).set({ role }).where(eq(users.id, user_id2));
}
async function deleteUser(user_id2) {
  const db2 = await getDb();
  await db2.delete(users).where(eq(users.id, user_id2));
}
async function getPaymentsByUserId(user_id2) {
  const db2 = await getDb();
  return db2.select().from(payments).where(eq(payments.user_id, user_id2)).orderBy(payments.created_at);
}
...
async function getAllCalendarEvents(startDate, endDate) {
  ...
  const bookingsData = await db2.select({
    id: bookings.id,
    pet_id: bookings.pet_id,
    petName: pets.name,
    bookingDate: bookings.bookingDate,
    numberOfDays: bookings.numberOfDays,
    status: bookings.status,
    notes: bookings.notes
  }).from(bookings).leftJoin(pets, eq(bookings.pet_id, pets.id)).where(
    and(
...
async function getWallPosts(limit = 20, offset = 0, petId2, userId2, targetType) {
  ...
  if (userId2) {
    const userInfo = await db2.select().from(users).where(eq(users.id, userId2));
    if (userInfo[0]?.role === "user") {
      const userPets = await db2.select().from(petTutors).where(eq(petTutors.tutor_id, user_id));
...
async function addWallReaction(postId, user_id2, reactionType) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
...
async function removeWallReaction(postId, user_id2) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
...
async function getConversations(user_id2) {
  const db2 = await getDb();
  if (!db2) return [];
...
async function markMessagesAsRead(conversationId, user_id2) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
...
async function getPetHistory(petId2) {
  ...
  return database.select().from(changeHistory).where(eq13(changeHistory.petId, petId2)).orderBy(changeHistory.createdAt);
}
...
async function getChangesByUser(userId2) {
  const { eq: eq13 } = await import("drizzle-orm");
  return database.select().from(changeHistory).where(eq13(changeHistory.changedBy, userId2)).orderBy(changeHistory.createdAt);
}
...
async function getActivityByDay(days = 30) {
  const { sql: sql4, desc: desc4 } = await import("drizzle-orm");
  const startDate = /* @__PURE__ */ new Date();
  startDate.setDate(startDate.getDate() - days);
  const allChanges = await database.select().from(changeHistory).where(sql4`${changeHistory.createdAt} >= ${startDate}`).orderBy(desc4(changeHistory.createdAt));
...
var init_changeTracker = __esm({
  "server/changeTracker.ts"() {
    "use strict";
    init_schema();
    client = postgres2(process.env.DATABASE_URL);
    database = drizzle2(client);
  }
});
...
async function changePassword(userId2, oldPassword, newPassword) {
  const db2 = await getDb();
  if (!db2) throw new Error("Database not available");
...
var init_sdk = __esm({
  "server/_core/sdk.ts"() {
    ...
    SDKServer = class {
      client;
      oauthService;
      constructor(client3 = createOAuthHttpClient()) {
        this.client = client3;
...
import { eq as eq8, and as and5, sql as sql3, inArray } from "drizzle-orm";
async function getAvailability(date2) {
  const MAX_CAPACITY = 10;
  const db2 = await getDb();
  const result = await db2.select({ count: sql3`count(*)` }).from(bookings).where(
    and5(
...
async function getTutorBookingRequests(tutorId2) {
  ...
  }).from(bookingRequests).leftJoin(pets, eq8(bookingRequests.petId, pets.id)).where(eq8(bookingRequests.tutorId, tutorId2)).orderBy(sql3`${bookingRequests.createdAt} DESC`);
  return requests;
}
async function getPendingBookingRequests() {
  ...
  const requests = await db2.select({
    ...
    notes: bookingRequests.notes,
    createdAt: bookingRequests.createdAt
  }).from(bookingRequests).leftJoin(pets, eq8(bookingRequests.petId, pets.id)).leftJoin(users, eq8(bookingRequests.tutorId, users.open_id)).where(eq8(bookingRequests.status, "pending")).orderBy(sql3`${bookingRequests.createdAt} ASC`);
  return requests;
}
async function getAllBookingRequests(status) {
  ...
  if (status) {
    const requests2 = await query.where(eq8(bookingRequests.status, status)).orderBy(sql3`${bookingRequests.createdAt} DESC`);
    return requests2;
  }
  const requests = await query.orderBy(sql3`${bookingRequests.createdAt} DESC`);
  return requests;
}
...
async function getBookingRequestStats() {
  ...
  const [stats] = await db2.select({
    total: sql3`count(*)`,
    pending: sql3`sum(case when ${bookingRequests.status} = 'pending' then 1 else 0 end)`,
    approved: sql3`sum(case when ${bookingRequests.status} = 'approved' then 1 else 0 end)`,
    rejected: sql3`sum(case when ${bookingRequests.status} = 'rejected' then 1 else 0 end)`,
    cancelled: sql3`sum(case when ${bookingRequests.status} = 'cancelled' then 1 else 0 end)`
...
var init_websocket = __esm({
  "server/_core/websocket.ts"() {
    ...
    init_sdk();
    init_schema();
    client2 = postgres3(process.env.DATABASE_URL);
    db = drizzle3(client2);
    io = null;
  }
});
...
async function getPrimaryTutorForPet(petId2) {
  ...
  }).from(petTutors3).where(eq13(petTutors3.pet_id, petId2));
...
async function handleCheckoutCompleted(session) {
  ...
  if (session.customer) {
    await db2.update(users).set({ stripe_customer_id: session.customer }).where(eq6(users.id, parseInt(userId2)));
  }
...
// server/routers.ts
var adminProcedure2 = protectedProcedure.use(async ({ ctx, next, path }) => {
