...
async function getResourceHistory(resourceType, resourceId) {
  ...
  return database.select().from(changeHistory).where(
    and8(
      eq14(changeHistory.resourceType, resourceType),
      eq14(changeHistory.resourceId, resourceId)
    )
  ).orderBy(changeHistory.createdAt);
}
async function getPetHistory(petId2) {
  const { eq: eq14 } = await import("drizzle-orm");
  return database.select().from(changeHistory).where(eq14(changeHistory.petId, petId2)).orderBy(changeHistory.createdAt);
}
...
async function getActivityByDay(days = 30) {
  const { sql: sql4, desc: desc5 } = await import("drizzle-orm");
  const startDate = /* @__PURE__ */ new Date();
  startDate.setDate(startDate.getDate() - days);
  const allChanges = await database.select().from(changeHistory).where(sql4`${changeHistory.createdAt} >= ${startDate}`).orderBy(desc5(changeHistory.createdAt));
...
async function changePassword2(userId2, oldPassword, newPassword) {
  try {
    const db2 = await getDb();
    if (!db2) throw new Error("Database not available");
    const userResults = await db2.select().from(users).where(eq6(users.id, userId2)).limit(1);
...
async function getOccupancyReport(startDate, endDate) {
  ...
  const { between, and: and8, isNotNull: isNotNull3, sql: sql4 } = await import("drizzle-orm");
  const MAX_CAPACITY = 20;
  const checkIns = await dbInstance.select({
    date: sql4`DATE(${daycareUsage2.check_in_time})`,
    count: sql4`COUNT(*)`,
    revenue: sql4`SUM(COALESCE(${daycareUsage2.amount_cents}, 0))`
  }).from(daycareUsage2).where(
    and8(
      between(daycareUsage2.check_in_time, startDate, endDate),
      isNotNull3(daycareUsage2.check_in_time)
    )
  ).groupBy(sql4`DATE(${daycareUsage2.check_in_time})`).orderBy(sql4`DATE(${daycareUsage2.check_in_time})`);
...
async function handleCheckoutCompleted(session) {
  ...
  if (session.customer) {
    await db2.update(users).set({ stripeCustomerId: session.customer }).where(eq7(users.id, parseInt(userId2)));
  }
...
async function handlePaymentSucceeded(paymentIntent) {
  ...
  if (productType === "credits" && creditsToAdd > 0) {
    const userPets = await db2.select().from(pets).where(eq7(pets.id, parseInt(userId2)));
...
async function getTutorBookingRequests(tutorId2) {
  ...
  }).from(bookingRequests).leftJoin(pets, eq8(bookingRequests.petId, pets.id)).where(eq8(bookingRequests.tutorId, tutorId2)).orderBy(sql2`${bookingRequests.createdAt} DESC`);
  return requests;
}
async function getPendingBookingRequests() {
  ...
  const requests = await db2.select({
    ...
    notes: bookingRequests.notes,
    createdAt: bookingRequests.createdAt
  }).from(bookingRequests).leftJoin(pets, eq8(bookingRequests.petId, pets.id)).leftJoin(users, eq8(bookingRequests.tutorId, users.open_id)).where(eq8(bookingRequests.status, "pending")).orderBy(sql2`${bookingRequests.createdAt} ASC`);
  return requests;
}
async function getAllBookingRequests(status) {
  ...
  let query = db2.select({
    ...
    approvedAt: bookingRequests.approvedAt,
    createdAt: bookingRequests.createdAt
  }).from(bookingRequests).leftJoin(pets, eq8(bookingRequests.petId, pets.id)).leftJoin(users, eq8(bookingRequests.tutorId, users.open_id));
  if (status) {
    const requests2 = await query.where(eq8(bookingRequests.status, status)).orderBy(sql2`${bookingRequests.createdAt} DESC`);
    return requests2;
  }
...
async function approveBookingRequest(requestId, adminId, adminNotes) {
  const db2 = await getDb();
  const [request] = await db2.select().from(bookingRequests).where(eq8(bookingRequests.id, requestId));
  if (!request) {
  ...
  const [pet] = await db2.select().from(pets).where(eq8(pets.id, request.petId));
  if (!pet) {
    throw new Error("Pet not found");
  }
  ...
  const [tutor] = await db2.select({ id: users.id }).from(users).where(eq8(users.open_id, request.tutorId));
  if (!tutor) {
  ...
  }).where(eq8(pets.id, request.petId));
  await db2.update(bookingRequests).set({
    ...
    adminNotes,
    updatedAt: /* @__PURE__ */ new Date()
  }).where(eq8(bookingRequests.id, requestId));
...
function initializeWebSocket(httpServer) {
  ...
  io.use(async (socket, next) => {
    ...
    try {
      ...
      const [user] = await db.select().from(users).where(eq9(users.open_id, session.openId)).limit(1);
      if (!user) {
...
async function getPrimaryTutorForPet(petId2) {
  ...
  }).from(petTutors3).where(eq14(petTutors3.pet_id, petId2));
  ...
  const tutors = await database2.select({
    name: users2.name,
    phone: users2.phone
  }).from(users2).where(eq14(users2.id, primaryRelation.tutorId));
...
async function getServicePrices() {
  const db2 = await getDb();
  return db2.select().from(servicePrices).where(eq10(servicePrices.isActive, true));
}
async function getServicePrice(serviceType) {
  const db2 = await getDb();
  const [price] = await db2.select().from(servicePrices).where(
    and6(
      eq10(servicePrices.serviceType, serviceType),
      eq10(servicePrices.isActive, true)
    )
  );
  return price;
}
async function getCustomPricingPlan(tutorId2) {
  ...
  const [plan] = await db2.select().from(customPricingPlans).where(
    and6(
      eq10(customPricingPlans.tutorId, tutorId2),
      eq10(customPricingPlans.isActive, true)
    )
...
async function updateCustomPricingPlan(id, data) {
  const db2 = await getDb();
  await db2.update(customPricingPlans).set(data).where(eq10(customPricingPlans.id, id));
}
async function deactivateCustomPricingPlan(id) {
  const db2 = await getDb();
  await db2.update(customPricingPlans).set({ isActive: false }).where(eq10(customPricingPlans.id, id));
}
...
async function addStock(amountKg, notes, createdBy) {
  ...
  if (stock.id > 0) {
    ...
    }).where(eq11(foodStock.id, stock.id));
  } else {
...
async function sendWhatsAppMessage(params) {
  ...
  const [config] = await db2.select().from((await Promise.resolve().then(() => (init_schema(), schema_exports))).whatsappConfig).where(
    eq12((await Promise.resolve().then(() => (init_schema(), schema_exports))).whatsappConfig.isActive, true)
  );
  ...
  try {
    ...
    for (const message of value.messages) {
      ...
      const [existing] = await db2.select().from(chatMessages).where(
        eq12(chatMessages.whatsappMessageId, whatsappMessageId)
      );
      if (existing) {
        continue;
      }
      const [user] = await db2.select().from(users).where(
        eq12(users.phone, fromPhone)
      );
      if (!user) {
      ...
      await db2.update(conversations).set({ lastMessageAt: new Date(timestamp2) }).where(eq12(conversations.id, conversation.id));
    }
...
async function syncMessageToWhatsApp(messageId) {
  ...
  await db2.update(chatMessages).set({ whatsappMessageId }).where(eq12(chatMessages.id, messageId));
...
async function checkUpcomingVaccines() {
  ...
  try {
    ...
    const upcomingVaccines = await db2.select({
      ...
      pet: pets,
      vaccine: vaccineLibrary
    }).from(petVaccinations).innerJoin(pets, eq4(petVaccinations.pet_id, pets.id)).innerJoin(vaccineLibrary, eq4(petVaccinations.vaccine_id, vaccineLibrary.id)).where(
      and2(
        isNotNull2(petVaccinations.next_due_date),
        gte2(petVaccinations.next_due_date, today),
        lte2(petVaccinations.next_due_date, sevenDaysFromNow)
      )
    );
    ...
    for (const { vaccination, pet, vaccine } of upcomingVaccines) {
      try {
        ...
        }).from(petTutors).innerJoin(users, eq4(petTutors.tutor_id, users.id)).where(eq4(petTutors.pet_id, pet.id)).orderBy(petTutors.is_primary);
    ...
    listPending: adminProcedure2.query(async () => {
      ...
      try {
        const result = await dbInstance.select({
          ...
          tutor: users,
          isPrimary: petTutors.is_primary
        }).from(pets).leftJoin(petTutors, eq13(pets.id, petTutors.pet_id)).leftJoin(users, eq13(petTutors.tutor_id, users.id)).where(eq13(pets.approval_status, "pending")).orderBy(desc4(pets.created_at));
...
`;
  ...
  healthCalendar: router({
    ...
    deleteBooking: adminProcedure2.input(z3.object({ id: z3.number() })).mutation(async ({ input }) => {
      ...
      const dbInstance = await getDb2();
      if (!dbInstance) throw new Error("Database not available");
      await dbInstance.delete(bookings2).where(eq14(bookings2.id, input.id));
      return true;
    }),
    /**
    ...
    updateBooking: adminProcedure2.input(z3.object({
      ...
      await dbInstance.update(bookings2).set({
        ...
        numberOfDays,
        notes: input.notes
      }).where(eq14(bookings2.id, input.id));
      return true;
    }),
    /**
     * Update transaction
  ...
  bookingRequests: router({
    ...
    approve: adminProcedure2.input(z3.object({
      ...
      const [request] = await db2.select().from(bookingRequests2).where(eq14(bookingRequests2.id, input.id));
