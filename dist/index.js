...
async function loginWithEmail(email, password) {
  try {
    ...
    if (error) {
      ...
      if (error.message.includes("Invalid login credentials") || error.message.includes("Email not confirmed") || error.message.includes("User not found") || error.message.includes("Invalid login")) {
...
async function getFoodStats() {
  ...
  cache.set(cacheKey, data, 30 * 1e3);
  return data;
}
